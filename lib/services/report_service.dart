...
// ============================================================
  ...
  Future<List<InfrastructureReport>> getWorkerReports(
      String workerId,
      ) async {
    final User? user = currentUser;
    ...
    final String cleanWorkerId = workerId.trim();

    if (cleanWorkerId.isEmpty) {
      throw Exception(
    ...
    try {
      ...
      return response
          .map(
            (item) => InfrastructureReport.fromMap(
          Map<String, dynamic>.from(
            item as Map,
          ),
        ),
...
// ============================================================
  ...
  Future<void> claimReport({
    ...
    if (cleanWorkerId.isEmpty) {
      throw Exception('Worker ID is required.');
    }
    ...
    try {
      ...
      existing['assigned_worker_id']?.toString();

      if (assignedWorkerId != null &&
          assignedWorkerId.isNotEmpty &&
          assignedWorkerId != cleanWorkerId) {
        throw Exception(
          'This task has already been claimed by another worker.',
        );
      }
      ...
      throw Exception(
        'Unable to claim task: ${_cleanError(e)}',
      );
    }
  }
  Future<void> uploadAfterRepairImage({
    required String reportId,
    ...
    if (!await imageFile.exists()) {
      throw Exception('Repair image could not be found.');
    }

    if (await imageFile.length() <= 0) {
      throw Exception('Repair image is empty.');
    }

    try {
      final String extension = _getExtension(imageFile.path);

      final String fileName =
          'after_repair_${DateTime.now().microsecondsSinceEpoch}.$extension';

      final String storagePath =
          'worker/${user.id}/$cleanReportId/$fileName';
      ...
      await _supabase
        ...
        'original_file_name': _fileNameFromPath(imageFile.path),
      ...
      throw Exception(
        'Unable to upload repair image: ${_cleanError(e)}',
      );
    }
  }
} // ← KEEP this final brace
